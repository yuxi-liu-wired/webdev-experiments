...
/** { rarity, tier, label } for a badge object from strict-lib's badges(). */
export function shininess(table, badgeObj) {
  ...
  const rarity = Math.round(table.scanned / Math.max(count, 1) * (count ? 1 : 2));
...
/** The rarest find of a batch: highest rarity, poem length breaks ties. */
export function pickRarest(table, finds) {
  let best = null;
  for (const f of finds) {
    const shine = shininess(table, f.badges);
